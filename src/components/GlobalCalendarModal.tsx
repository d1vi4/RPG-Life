...
export const GlobalCalendarModal: React.FC<GlobalCalendarModalProps> = ({
   ...
   * STRICT FIX: If dateStr is before task.createdAt, do NOT include this task!
   */
  ...
  const content = (
    <div
      className={`w-full bg-[#0F172A] border border-slate-800 rounded-2xl shadow-xl text-slate-100 flex flex-col overflow-hidden ${
        asEmbeddedView ? '' : 'max-h-[92vh] max-w-5xl'
      }`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-800 bg-[#0B0F19]/70 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 hidden sm:block">
              Нажмите на любой день, чтобы просмотреть задачи по всем направлениям
            </p>
          </div>
        </div>
      ...
      {/* Body Grid: Calendar (left) + Day Breakdown (right) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 flex-1 overflow-y-auto min-h-0">
        {/* Left: Full Month Calendar */}
        <div className="lg:col-span-7 p-4 sm:p-5 border-b lg:border-b-0 lg:border-r border-slate-800 flex flex-col space-y-3.5">
            ...
            {calendarDays.map((cell) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-[50px] sm:min-h-[60px] p-1.5 rounded-xl border flex flex-col justify-between transition-all cursor-pointer text-left relative ${statusBorder} ${
        ...
        {/* Right: Selected Day Breakdown by Category */}
        <div className="lg:col-span-5 p-4 sm:p-5 flex flex-col space-y-3.5 bg-[#0B0F19]/40 overflow-y-auto">
          ...
          {categoriesWithTasksOnDate.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-slate-500 border border-dashed border-slate-800 rounded-2xl">
              ...
              <p className="text-xs text-slate-600 mt-1 max-w-xs">
                Перейдите в нужное направление на вкладке «Категории», чтобы добавить задачи.
              </p>
            </div>
          ) : (
